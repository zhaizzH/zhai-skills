---
name: aivedio-cover
description: GPT Image 2 图像生成 / 编辑技能。3 种运行模式：(A) Garden 本地出图并落盘；(B) 把渲染好的 prompt 交给宿主图像工具出图；(C) 无图像工具时退化为高质量 prompt 顾问。18 大类 80+ 结构化模板（海报 / UI 样机 / 产品图 / 信息图 / 学术图 / 架构图 / 漫画 / 头像 / 分镜 / IP 周边 / 编辑工作流等）。**用户说"出图/改图"就优先用它**：做封面、出缩略图、生成视频封面、加张配图、生成海报、画 UI 示意图、论文配图、产品主图、头像、出一张 xxx 图。也是 aivedio-video 的视频封面 / 配图能力提供方（套 weekly-cover-design.md，swiss-ikb 克莱因蓝，3:4 1080x1440）。**触发词**：做封面 / 出缩略图 / 生成视频封面 / 加张配图 / 图像生成 / 出一张图 / 画个图 / 生成海报 / aivedio-cover。任何任务第一步先跑 scripts/check-mode.js 定 A/B/C 模式。
---

# aivedio-cover

> **位置**：本 skill 位于 `.claude/skills/aivedio-cover/`（AiVedio 项目根下，与 aivedio-video 平级），是独立的可调用 skill，也是 aivedio-video 封面 / 配图能力的提供方。
> **路径约定**：下文示例命令中的 `<skill>` 统一指 `.claude/skills/aivedio-cover`（相对项目根 AiVedio）。若当前工作目录就是项目根（weekly 流程通常如此），直接把 `<skill>` 换成 `.claude/skills/aivedio-cover` 即可照抄；否则用绝对路径。
> 脚本默认输出仍相对当前工作区（`garden-aivedio-cover/`）；在 weekly 流程中，建议用 `--prompt-output` / `--image` 把产物重定向到 `weekly/weekly-<N>/`。
...
## aivedio-video 集成（封面 / 配图）

本 skill 同时服务 aivedio-video：做 weekly 视频封面 / 配图时由它调用本 skill（用户说"做封面 / 出缩略图 / 生成视频封面 / 加张配图"）。此时**风格锁死 swiss-ikb**，比下方通用模板更严格：

- **封面权威源**：`../aivedio-video/references/weekly-cover-design.md`（相对本 skill 目录）—— 打开后按其 Prompt 模板构造，仅替换 `[主标题]` / `[副标题]` 占位符
- **尺寸**：封面 3:4 `1080x1440`；配图按需其它比例（如 16:9），仍遵守下方禁止项
- **硬性禁止**（weekly 封面一律不可违反）：无照片、无渐变、无阴影、无圆角、无 emoji；纯色净暖白底（`#F5F0EB`）+ 克莱因蓝（`#002FA7`）几何元素 + 超细无衬线排版
- **落盘**：Mode A 下用 `--image weekly/weekly-<N>/cover.png --prompt-output weekly/weekly-<N>/cover-prompt.md` 显式指定；B / C 至少保证 prompt 落盘到 `weekly/weekly-<N>/`
- 即便从 18 大类通用模板里补充画面细节，也必须守住上面的 swiss-ikb 约束

非 weekly 的普通出图需求（海报 / UI / 信息图 / 学术图等）仍走下方通用模板索引。
...
```bash
node <skill>/scripts/check-mode.js
# 想拿结构化结果给上层程序用：
node <skill>/scripts/check-mode.js --json
```
...
- prompt 默认落盘到 `garden-aivedio-cover/prompt/`、图片落盘到 `garden-aivedio-cover/image/`。
- 这是最强的模式：你是图像工具的"持有者"。
...
4. 如用户希望可顺手把 prompt 文件保存到 `garden-aivedio-cover/prompt/`，但图片去向由宿主决定，不强制。
...
2. 把最终 prompt **直接打印给用户** + 保存一份到 `garden-aivedio-cover/prompt/<task-slug>-<timestamp>.md`。
...
- 提示词目录：`garden-aivedio-cover/prompt/`（**A / B / C 三种模式都建议用**，方便复用与版本管理）
- 图片目录：`garden-aivedio-cover/image/`（**仅 Mode A 使用**；Mode B 由宿主决定，Mode C 不产生图）
...
- 提示词：`garden-aivedio-cover/prompt/<task-slug>-<timestamp>.md`
- 图片：`garden-aivedio-cover/image/<task-slug>-<timestamp>.png`
...
- `garden-aivedio-cover/prompt/live-commerce-ui-20260424-153045.md`
- `garden-aivedio-cover/image/live-commerce-ui-20260424-153045.png`
- `garden-aivedio-cover/prompt/vr-headset-exploded-view-20260424-153102.md`
- `garden-aivedio-cover/image/vr-headset-exploded-view-20260424-153102.png`
...
2. 如果用户直接给的是文本 prompt，也要先把最终 prompt 保存到 `garden-aivedio-cover/prompt/`。
...
2. 否则默认保存到 `garden-aivedio-cover/image/`。
...
```bash
node <skill>/scripts/generate.js \
...
```bash
node <skill>/scripts/generate.js \
  --promptfile garden-aivedio-cover/prompt/poster-20260424-153045.md
```

### 3. 编辑已有图片（Mode A）

```bash
node <skill>/scripts/edit.js \
...
- **Mode C**：渲染好最终 prompt → 保存到 `garden-aivedio-cover/prompt/<task-slug>-<timestamp>.md` → 把内容直接展示给用户 → 提示用户在哪些图像工具中可以直接复用。
...
7-A. **Mode A**：把最终 prompt 保存到 `garden-aivedio-cover/prompt/`，调用 `scripts/generate.js` 或 `scripts/edit.js`，图片落到 `garden-aivedio-cover/image/`。
7-B. **Mode B**：把最终 prompt 直接传给宿主的图像工具调用；按需保存 prompt 副本到 `garden-aivedio-cover/prompt/`。
7-C. **Mode C**：把最终 prompt 保存到 `garden-aivedio-cover/prompt/<task-slug>-<timestamp>.md`，并把完整 prompt 在对话中展示给用户，附一句简短的"如何使用 / 推荐工具"建议。
