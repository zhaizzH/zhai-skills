...
> **位置**：本 skill 位于 `.claude/skills/aivedio-cover/`（AiVedio 项目根下，与 aivedio-video 平级）。下文示例命令里的 `<skill>` 统一指 `.claude/skills/aivedio-cover`（相对项目根）。
...
- **可复用的 prompt + 图片归档**：默认落盘到 `garden-aivedio-cover/prompt/` 和 `garden-aivedio-cover/image/`，按 `<task-slug>-<timestamp>` 命名。
...
| **C · Advisor 纯提示词顾问** | 未启用 Garden，宿主也没有图像工具 | 退化成"高质量 prompt 撰写顾问"——把 prompt 落盘到 `garden-aivedio-cover/prompt/`，告诉用户去 ChatGPT / Midjourney / DALL·E / Sora / Nano Banana / 自己的网关里执行 |
...
| 内容 | 落盘位置 | 适用模式 |
|---|---|---|
| 渲染好的 prompt | `garden-aivedio-cover/prompt/<task-slug>-<timestamp>.md` | A / B / C |
| 生成的图片 | `garden-aivedio-cover/image/<task-slug>-<timestamp>.png` | 仅 A（B 由宿主决定，C 不产出） |
