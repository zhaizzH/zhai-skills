...
> **Location**: this skill lives at `.claude/skills/aivedio-cover/` (project root, sibling of `aivedio-video`). In the examples below, `<skill>` = `.claude/skills/aivedio-cover` relative to the project root.
...
- **Reproducible prompt + image archival** under `garden-aivedio-cover/prompt/` and `garden-aivedio-cover/image/` with task-slug + timestamp naming.
...
| **C — Advisor** | Garden disabled, host has no image tool | Skill degrades into a high-quality prompt writer — saves the rendered prompt to `garden-aivedio-cover/prompt/` and instructs the user to paste it into ChatGPT / Midjourney / DALL·E / Sora / Nano Banana / their own gateway |
...
| What | Where | Used in |
|---|---|---|
| Rendered prompts | `garden-aivedio-cover/prompt/<task-slug>-<timestamp>.md` | A / B / C |
| Generated images | `garden-aivedio-cover/image/<task-slug>-<timestamp>.png` | A only (B = host decides, C = none) |
